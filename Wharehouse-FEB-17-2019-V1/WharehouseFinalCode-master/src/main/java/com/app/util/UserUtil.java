...
public class UserUtil {
	...
	public void generatePieChart(String path,List<Object[]> users) {
		...
		//3.save as jpeg
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/UsersPie.jpg"), jFreeChart, 250, 250);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	public void generateBarChart(String path,List<Object[]> users) {
		...
		try {
			ChartUtils.saveChartAsJPEG(new File(path+"/resources/images/UsersBar.jpg"), jFreeChart, 250, 250);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
