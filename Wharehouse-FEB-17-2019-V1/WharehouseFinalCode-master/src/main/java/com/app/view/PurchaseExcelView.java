...
public class PurchaseExcelView extends AbstractXlsxView{
	...
	private void setBody(Sheet sheet, List<Purchase> purchase) {
		...
		for(Purchase p:purchase) {
		    ...
		    row.createCell(2).setCellValue(p.getRefNumber());
			row.createCell(3).setCellValue(p.getQuaCheck());
			row.createCell(4).setCellValue(p.getOrderStatus());
			row.createCell(5).setCellValue(p.getOrderDesc());
			row.createCell(6).setCellValue(p.getWhUserType().getUserName());
